!(function (A, t) {
	if ('object' == typeof exports && 'object' == typeof module)
		...
		var e = t()
		for (var g in e) ('object' == typeof exports ? exports : A)[g] = e[g]
	}
})(self, () => {
	return (
		(A = {
					...
					D = (A.exports = {
						initialize: function () {
							for (var A in D) {
								var t = D[A]
								if (null != t.spacing)
									for (var e in ((t.characters = {}), t.base64)) {
										var w = g.decode(t.base64[e])
										t.characters[e] = []
										for (var P = 0; P < t.size.height; P++) {
											t.characters[e][P] = []
											for (var i = 0; i < t.size.width; i++) {
												var h = P * t.size.width + i
												t.characters[e][P].push(w[h])
											}
										}
									}
							}
						...
						A: {
							name: 'FONT_A',
							spacing: { right: 2, left: 0, top: 0, bottom: 0 },
						...
						B: {
							name: 'FONT_B',
							spacing: { right: 0, left: 0, top: 0, bottom: 0 },
			...
			241: (A) => {
				...
				A.exports = {
					encode: function (A) {
						let e = ''
						const g = (6 - (A.length % 6)) % 6,
							D = (24 - ((A.length + g) % 24)) / 6
						for (let t = 0; t < g; t++) A.push(0)
						for (let g = 0; g < A.length; g += 6) {
							let D = 0
							for (let t = 0; t < 6; t++)
								D += A[g + t] > 0 ? Math.pow(2, 5 - t) : 0
							e += t[D]
						}
						for (let A = 0; A < D; A++) e += '='
						return e
					...
					decode: function (A) {
						...
						for (let g = 0; g < A.length; g++) {
							let D = '000000' + t.indexOf(A[g]).toString(2)
							D = D.substring(D.length - 6)
							for (let A = 0; A < D.length; A++) e.push('0' == D[A] ? 0 : 1)
						}
						return e
					}
				}
				...
				A.exports = class extends g {
					constructor() {
						super(),
							...
							(this.dataPrepend = ''),
							(this.maxLength = 32),
							(this.type = new D(w.CODE_11)),
							(this.subset = ''),
							(this.interpretationLine = !0),
							(this.notImplemented = ['typeName', 'invert'])
					}
					generateZPL(A, t, e, g, D, P) {
						const i = this.getPosition(A, t, e, g, D, P)
						let h,
							H = ''
						switch (
							(this.type.value === w.Code49
								? (H +=
										'^FO' + Math.round(i.left) + ',' + Math.round(i.top + 25))
								: (H += '^FO' + Math.round(i.left) + ',' + Math.round(i.top)),
							this.invert && (H += '^FR'),
							this.type.value)
						) {
							case w.Code11:
								H +=
									...
									i.height +
									',' +
									(this.interpretationLine ? 'Y' : 'N') +
									',N'
								break
							case w.Interleaved25:
								H +=
									'^B2N,' +
									i.height +
									',' +
							...
							case w.PDF417:
								const A = 10
								h = Math.ceil(i.height / A)
								const t = this.maxLength * h
								H += '^B7N,' + A + ',0,' + Math.ceil(t / h) + ',' + h + ',N'
								break
							case w.EAN8:
								H +=
									'^B8N,' +
							...
							case w.QRCode:
								;(H +=
									'^BQ,2,' + Math.min(Math.floor(i.height / 25), 10) + ',Q,7'),
									'' === this.dataPrepend && (this.dataPrepend = 'QA,')
								break
							case w.DataMatrix:
								H += '^BXN,10,200,,,~,1'
								break
						...
						return (
							(H += '^FD' + this.dataPrepend + this.data), (H += '^FS\n'), H
						)
					}
					generateBinaryImage(A, t, e, g, D, w, i) {
						const h = this.getPosition(t, e, g, D, w, i),
							H = P.BarcodeRenderer.render(
								h.width,
								h.height,
								this.type,
								this.data
							)
						for (let t = 0; t < h.height; t++)
							for (let e = 0; e < h.width; e++) {
								const g = t + h.top,
									D = e + h.left
								A[g][D] = H[t][e]
							}
			...
			338: (A) => {
				A.exports = class {
					...
					generateZPL(A, t, e, g, D, w) {
						return ''
					}
					generateXML(A, t) {
						return ''
					}
					generateBinaryImage(A, t, e, g, D, w, P) {}
				}
			},
			96: (A, t, e) => {
				const g = e(203),
				...
				A.exports = class extends g {
					constructor() {
						super(), (this.padding = new D()), (this.content = [])
					}
					calculateSizing(A, t, e, g) {
						const D = this.calculateUnits(),
							w = this.margin.left + this.padding.left,
							P = this.margin.top + this.padding.top,
							i = w + this.margin.right + this.padding.right,
							h = P + this.margin.bottom + this.padding.right,
							H = A - i - 2 * (this.border || 0),
							s = t - h - 2 * (this.border || 0)
						return {
							spacingTop: P,
							spacingLeft: w,
							width: H,
							height: s,
							widthUnits: (H - D.absolute.width) / (D.relative.width || 1),
							heightUnits: (s - D.absolute.height) / (D.relative.height || 1)
						}
					}
					generateZPL(A, t, e, g, D, w) {
						const P = this.calculateSizing(e, g, D, w)
						let i = ''
						for (let e in this.content) {
							const g = this.content[e]
							let D = A + P.spacingLeft + (this.border || 0),
								w = t + P.spacingTop + (this.border || 0)
							g.fixed &&
								((D = this.getSize(g.left)), (w = this.getSize(g.top))),
								(i += g.generateZPL(
									D,
									w,
									P.width,
					...
					generateBinaryImage(A, t, e, g, D, w, P) {
						const i = this.calculateSizing(g, D, w, P)
						for (let g in this.content) {
							const D = this.content[g]
							let w = t + i.spacingLeft + (this.border || 0),
								P = e + i.spacingTop + (this.border || 0)
							D.fixed &&
								((w = this.getSize(D.left)), (P = this.getSize(D.top))),
								D.generateBinaryImage(
									A,
									w,
									P,
									i.width,
			...
			48: (A, t, e) => {
				...
				A.exports = class extends g {
					constructor() {
						super(), (this.border = 0), (this.fill = !1)
					}
				}
			},
			203: (A, t, e) => {
				...
				A.exports = class extends g {
					...
					getPosition(A, t, e, g, D, w) {
						let P = this.getSize(this.left, D) + this.margin.left,
							h = this.getSize(this.top, w) + this.margin.top
						const H = this.getSize(this.width, D) || e - this.margin.horizontal,
							s = this.getSize(this.height, w) || g - this.margin.vertical
						return (
							'object' == typeof this.top &&
								this.top.sizeType == i.Fraction &&
								(h = g * this.top.value),
							'object' == typeof this.left &&
								this.left.sizeType == i.Fraction &&
								(P = e * this.left.value),
							{
								left: Math.round(P + A),
								top: Math.round(h + t),
					...
					getSize(A, t) {
						return 'number' == typeof A ? A : A.getValue(t)
					}
					calculateUnits() {
						const A = {
						...
						for (let e of t)
							e.margin &&
								...
								e.width &&
								e.height &&
								((A.absolute.width += e.margin.horizontal + (this.border || 0)),
								(A.absolute.height += e.margin.vertical + (this.border || 0)),
								'number' == typeof e.border &&
									((A.absolute.width += 2 * e.border),
									(A.absolute.height += 2 * e.border)),
								'number' == typeof e.width
									? (A.absolute.width += e.width)
									: e.width.sizeType == i.Absolute
									? (A.absolute.width += e.width.value)
									: (A.relative.width += e.width.value),
								'number' == typeof e.height
									? (A.absolute.height += e.height)
									: e.height.sizeType == i.Absolute
									? (A.absolute.height += e.height.value)
									: (A.relative.height += e.height.value))
						return A
					}
				}
			},
			24: (A, t, e) => {
				...
				A.exports = class extends g {
					constructor() {
						super(), (this.typeName = 'Box'), (this.cornerRadius = 0)
					}
					generateZPL(A, t, e, g, D, w) {
						const P = this.getPosition(A, t, e, g, D, w)
						let i = '^FO' + Math.round(P.left) + ',' + Math.round(P.top)
						this.invert && (i += '^FR')
						let h = this.border
						this.fill && (h = Math.min(P.width, P.height))
						const H = Math.min(P.width, P.height),
							s = Math.round((16 * this.cornerRadius) / H)
						return (
							(i +=
								h > 0
									? '^GB' +
									  ...
									  '^FS\n'
									: '\n'),
							(i += super.generateZPL(P.left, P.top, P.width, P.height)),
							i
						)
					}
					generateBinaryImage(A, t, e, g, D, w, P) {
						const i = this.getPosition(t, e, g, D, w, P),
							h = this.border || 0,
							H = this.cornerRadius,
							s = i.height - this.cornerRadius - 1,
							n = this.cornerRadius,
							B = i.width - this.cornerRadius - 1
						if (this.fill || this.border > 0)
							for (let t = 0; t < i.height; t++)
								for (let e = 0; e < i.width; e++) {
									const g = e + i.left,
										D = t + i.top
									if (D < 0 || g < 0 || D >= A.length || g >= A[D].length)
										continue
									let w
									if (
										(this.cornerRadius > 0 &&
										...
										(this.fill ||
											...
											e < h ||
											e >= i.width - h) &&
											(this.invert ? (A[D][g] = !A[D][g]) : (A[D][g] = !0))
									else {
										const P = Math.sqrt(
											Math.pow(t - w.y, 2) + Math.pow(e - w.x, 2)
										)
										P <= this.cornerRadius + 1 &&
											(this.fill || P >= this.cornerRadius - this.border) &&
											(this.invert ? (A[D][g] = !A[D][g]) : (A[D][g] = !0))
									}
								}
						super.generateBinaryImage(A, i.left, i.top, i.width, i.height)
					}
				}
			},
			753: (A, t, e) => {
				...
				A.exports = class extends g {
					...
					generateBinaryImage(A, t, e, g, D, w, P) {
						...
						if (this.fill || this.border > 0)
							for (let t = 0; t < i.height; t++)
								for (let e = 0; e < i.width; e++) {
									const g = Math.round(t + i.top),
										D = Math.round(e + i.left)
									let w = !1
									;(w =
										Math.pow(e - H, 2) / Math.pow(s, 2) +
											Math.pow(t - h, 2) / Math.pow(n, 2) <=
										1),
										0 == this.fill &&
											(w &=
												Math.pow(e - H, 2) / Math.pow(B, 2) +
													Math.pow(t - h, 2) / Math.pow(r, 2) >=
												1),
										w && (this.invert ? (A[g][D] = !A[g][D]) : (A[g][D] = w))
								}
						super.generateBinaryImage(A, i.left, i.top, i.width, i.height)
					}
				}
			...
			693: (A, t, e) => {
				...
				A.exports = class extends g {
					...
					extractImageData(A) {
						A((i.ImageProcessor || new w()).processImage(this.data))
					}
					generateZPL(A, t, e, g, D, w) {
						let P = this.generateContainer().generateZPL(A, t, e, g, D, w)
						const H = this.getPosition(A, t, e, g, D, w),
							s = i.ImageResizer.resize(
								H.width,
								H.height,
								this.data.width,
								this.data.height,
						...
						;(P += '^FO' + Math.round(H.left) + ',' + Math.round(H.top)),
							this.invert && (P += '^FR')
						const n = Math.ceil(H.width / 8),
							B = n * H.height
						let r = h.generateHexAscii(H.width, H.height, s)
						return (
							(r = h.encodeHexAscii(r)),
							(P += '^GFA,' + B + ',' + B + ',' + n + ',' + r + '^FS\n'),
							P
						)
					}
					generateBinaryImage(A, t, e, g, D, w, P) {
						this.generateContainer().generateBinaryImage(A, t, e, g, D, w, P)
						const h = this.getPosition(t, e, g, D, w, P),
							H = i.ImageResizer.resize(
								h.width,
								h.height,
								this.data.width,
								this.data.height,
						...
						for (let t = 0; t < h.height; t++)
							for (let e = 0; e < h.width; e++) {
								const g = t + h.top,
									D = e + h.left
								H[t * h.width + e] &&
									(this.invert ? (A[g][D] = !A[g][D]) : (A[g][D] = !0))
							}
					}
				}
			},
				...
				A.exports = class extends g {
					...
					calculateSizing(A, t, e, g) {
						const D = this.calculateUnits(),
							w = this.margin.left,
							P = this.margin.top,
							i = A - (w + this.margin.right),
							h = t - (P + this.margin.bottom)
						return {
							spacingTop: P,
							spacingLeft: w,
							width: i,
							height: h,
							widthUnits: (i - D.absolute.width) / (D.relative.width || 1),
							heightUnits: (h - D.absolute.height) / (D.relative.height || 1)
						}
					}
					generateChildren(A, t) {
						const e = this.columns
						0 == e.length && e.push(new w(1, i.Relative))
						const g = this.rows
						0 == g.length && g.push(new w(1, i.Relative))
						const h = {
							absolute: { width: 0, height: 0 },
							relative: { width: 0, height: 0 }
						}
						for (let A of e)
							'object' == typeof A
								? A.sizeType == i.Absolute
									? (h.absolute.width += A.value)
									: (h.relative.width += A.value)
								: 'number' == typeof A && (h.absolute.width += A)
						for (let A of g)
							'object' == typeof A
								? A.sizeType == i.Absolute
									? (h.absolute.height += A.value)
									: (h.relative.height += A.value)
								: 'number' == typeof A && (h.absolute.height += A)
						const H = 4 * (this.border || 0)
						;(h.absolute.width += H), (h.absolute.height += H)
						const s = A - H - this.columnSpacing * (e.length + 1),
							n = t - H - this.rowSpacing * (g.length + 1),
							B = (s - h.absolute.width) / (h.relative.width || 1),
							r = (n - h.absolute.height) / (h.relative.height || 1),
							f = [],
							c = []
						let o = this.rowSpacing,
							a = n + 2 * (this.border || 0)
						for (let A = 0; A < g.length; A++) {
							f[A] = []
							let t = s + 2 * (this.border || 0),
								w = this.columnSpacing,
								P = Math.ceil(this.getSize(g[A], r)) + (this.border || 0)
							A == this.rows.length - 1 && (P = a), (a -= P)
							for (let g = 0; g < this.columns.length; g++) {
								const i = new D()
								f[A].push(i), c.push(i)
								let h = Math.ceil(this.getSize(e[g], B)) + (this.border || 0)
								g == this.columns.length - 1 && (h = t),
									(t -= h),
									(i.width = h),
									(i.height = P),
									(i.top = o),
									(i.left = w),
									(i.border = this.border),
									(i.padding = this.padding),
									(w += h + this.columnSpacing)
							}
							o += P + this.rowSpacing
						}
						for (let A of this.content)
							A.grid &&
								(A.grid.row < 0 ||
									A.grid.row >= g.length ||
									A.grid.column < 0 ||
									A.grid.column >= e.length ||
									f[A.grid.row][A.grid.column].content.push(A))
						const l = new D()
						return (
							(l.content = c),
							(l.fixed = this.fixed),
							...
							(l.border = this.border),
							(l.invert = this.invert),
							(l.padding = new P()),
							l
						)
					}
					generateZPL(A, t, e, g, D, w) {
						const P = this.getPosition(A, t, e, g, D, w),
							i = this.generateChildren(P.width, P.height),
							h = this.calculateSizing(e, g, D, w)
						return i.generateZPL(
							P.left,
							P.top,
							h.width,
							h.height,
					...
					generateBinaryImage(A, t, e, g, D, w, P) {
						const i = this.getPosition(t, e, g, D, w, P),
							h = this.generateChildren(i.width, i.height),
							H = this.calculateSizing(g, D, w, P)
						h.generateBinaryImage(
							A,
							i.left,
							i.top,
							H.width,
			...
			155: (A, t, e) => {
				...
				A.exports = class extends g {
					constructor() {
						super(),
							(this.typeName = 'Label'),
							(this.printDensity = new D(w['8dpmm'])),
							(this.notImplemented = [
								'typeName',
								'fixed',
								'grid',
					...
					generateZPL() {
						let A = '^XA'
						A += '\n'
						const t = this.getSize(this.width) * this.printDensity.value,
							e = this.getSize(this.height) * this.printDensity.value
						return (A += super.generateZPL(0, 0, t, e)), (A += '^XZ'), A
					}
					generateBinaryImage(A) {
						const t = this.getSize(this.width) * this.printDensity.value,
							e = this.getSize(this.height) * this.printDensity.value
						for (let g = 0; g < e; g++) {
							A.push([])
							for (let e = 0; e < t; e++) A[g].push(!1)
						}
						super.generateBinaryImage(A, 0, 0, t, e)
					}
				}
			},
			963: (A, t, e) => {
				...
				A.exports = class extends g {
					...
					generateZPL(A, t, e, g, D, w) {
						...
						let i =
							'^FO' +
							Math.round(P.left + Math.min(this.x1, this.x2)) +
							',' +
							Math.round(P.top + Math.min(this.y1, this.y2))
						this.invert && (i += '^FR')
						...
						return (
							((this.x1 < this.x2 && this.y1 < this.y2) ||
								...
								(s = 'L'),
							(i += `^GD${h},${H},${this.thickness},B,${s}^FS\n`),
							(i += super.generateZPL(P.left, P.top, P.width, P.height)),
							i
						)
					}
					generateBinaryImage(A, t, e, g, D, w, P) {
						const i = this.getPosition(t, e, g, D, w, P)
						if (this.thickness > 0) {
							const t = this.y1 < this.y2 ? 1 : -1,
								e = this.x1 < this.x2 ? 1 : -1,
								g = Math.atan2(this.y1 - this.y2, this.x1 - this.x2)
							for (let D = this.y1; D != this.y2; D += t)
								for (let t = this.x1; t != this.x2; t += e) {
									const e = t + i.left,
										w = D + i.top
									if (w < 0 || e < 0 || w >= A.length || e >= A[w].length)
										continue
									const P = Math.sqrt(
											Math.pow(this.x1 - t, 2) + Math.pow(this.y1 - D, 2)
										),
										h = Math.atan2(this.y1 - D, this.x1 - t)
									Math.abs(Math.sin(h - g) * P) > this.thickness / 2 ||
										(this.invert ? (A[w][e] = !A[w][e]) : (A[w][e] = !0))
								}
						}
						super.generateBinaryImage(A, i.left, i.top, i.width, i.height)
					}
			...
			919: (A, t, e) => {
				...
				A.exports = class extends g {
					constructor() {
						super(), (this.typeName = 'Raw'), (this.data = '')
					}
					generateZPL(A, t, e, g, D, w) {
				...
				A.exports = class extends g {
					constructor() {
						super(),
							...
							(this.format = '0001'),
							(this.increment = 1),
							(this.printLeadingZeroes = !0)
					}
					getTextLines() {
						const A = new RegExp('\\\\r\\\\n|\\\\n', 'g')
						return this.format.replace(A, '\n').split('\n')
					}
					generateZPL(A, t, e, g, w, P) {
						const i = this.getPosition(A, t, e, g, w, P)
						let h,
							H = '',
							s = ''
						...
						return (
							...
							(H +=
								...
								(this.characterWidth || '') +
								',\n'),
							(H += '^FB' + Math.round(i.width) + ',1000,0,' + h + ',0\n'),
							(H +=
								'^SN' +
								this.format +
								s +
				...
				A.exports = class extends g {
					constructor() {
						super(),
							(this.typeName = 'Text'),
							(this.text = ''),
							(this.fontFamily = new D(P.A)),
							(this.lineSpacing = 0),
							(this.verticalAlignment = new w(i.Start)),
							(this.horizontalAlignment = new w(i.Start)),
					...
					characterMap() {
						const A = [],
							t = this.fontFamily.definition.characters,
							e = this.getTextLines()
						for (let g of e) {
							const e = []
							A.push(e)
							for (let A of g) void 0 === t[A] && (A = ' '), e.push(t[A])
						}
						return A
					}
					calculateSize() {
						const A = this.characterMap(),
							t =
					...
					generateZPL(A, t, e, g, D, w) {
						const P = this.getPosition(A, t, e, g, D, w),
							h = this.calculateSize()
						this.verticalAlignment.value == i.End
							? (P.top = P.top + P.height - h.height)
							: this.verticalAlignment.value == i.Center &&
							  (P.top = P.top + P.height / 2 - h.height / 2)
						let H,
							s = ''
						this.invert && (s += '^LRY\n')
						let n = ''
						...
						const B = this.getTextLines()
						let r = 0
						for (let A of B)
							(s += '^FO' + Math.round(P.left) + ',' + Math.round(P.top + r)),
								(s +=
									'^A' +
									this.fontFamily.value +
									',' +
									...
									(this.characterWidth || '') +
									',\n'),
								(s += '^FB' + Math.round(P.width) + ',1000,0,' + H + ',0\n'),
								(s += '^FD' + A + n + '^FS\n'),
								(r += this.fontFamily.definition.size.height + this.lineSpacing)
						return this.invert && (s += '^LRN\n'), s
					}
					generateBinaryImage(A, t, e, g, D, w, P) {
						const h = this.getPosition(t, e, g, D, w, P),
							H = this.characterMap(),
							s = this.calculateSize()
						this.horizontalAlignment == i.End
							? (h.left = h.left + h.width - s.width)
							: this.horizontalAlignment == i.Center &&
							  (h.left = h.left + (h.width - s.width) / 2),
							this.verticalAlignment.value == i.End
								? (h.top = h.top + h.height - s.height)
								: this.verticalAlignment.value == i.Center &&
								  (h.top = h.top + h.height / 2 - s.height / 2)
						let n = 0
						for (let t of H) {
							let e = 0
							for (let g of t) {
								...
								for (let e = 0; e < g.length; e++)
									for (let w = 0; w < g[0].length; w++) {
										const P = 1 == g[e][w],
											i = Math.round(e + t),
											h = Math.round(w + D)
										0 != (i > 0 && i < A.length && h > 0 && h < A[i].length) &&
											P &&
											(this.invert ? (A[i][h] = !A[i][h]) : (A[i][h] = P))
									}
							}
							n += this.fontFamily.definition.size.height + this.lineSpacing
						}
					}
				}
			},
			408: (A) => {
				A.exports = { Start: 'Start', Center: 'Center', End: 'End' }
			},
			28: (A) => {
				A.exports = {
					Code11: 'Code11',
			...
			969: (A) => {
				A.exports = { A: 'A', B: 'B', C: 'C', D: 'D', E: 'E', F: 'F' }
			},
			767: (A) => {
				A.exports = { '6dpmm': 6, '8dpmm': 8, '12dpmm': 12, '24dpmm': 24 }
			},
			643: (A) => {
				A.exports = { Normal: 'N', Right: 'R', Bottom: 'I', Left: 'B' }
			},
			345: (A) => {
				A.exports = { Absolute: 0, Fraction: 1, Relative: 2 }
			},
			192: (A, t, e) => {
				const g = e(24),
					D = e(6),
				...
				A.exports = class {
					...
					render(A, t, e, H) {
						const s = new g()
						;(s.width = A), (s.height = t), (s.border = 2)
						const n = new D()
						s.content.push(n),
							(n.fontFamily = new h(w.B)),
							(n.text = 'BARCODE'),
							(n.verticalAlignment = new i(P.Center)),
							(n.horizontalAlignment = new i(P.Center)),
							(H = [])
						for (let e = 0; e < t; e++) {
							H.push([])
							for (let t = 0; t < A; t++) H[e].push(0)
						}
						return s.generateBinaryImage(H, 0, 0, A, t, A, t), H
					}
				}
			},
			202: (A) => {
				A.exports = class {
					...
					processImage(A, t) {
						LabelTools.Logger('Image Processor not defined'), t([])
					}
					processZplImage(A, t, e) {}
				}
			},
			940: (A) => {
				A.exports = class {
					...
					resize(A, t, e, g, D) {
						...
						for (let g = 0; g < t; g++)
							for (let t = 0; t < A; t++) {
								const A = D[Math.floor(i * g) * e + Math.floor(P * t)]
								w.push(A ? 1 : 0)
							}
						return w
					}
			...
			558: (A) => {
				A.exports = {
					generateHexAscii: (A, t, e) => {
						...
						for (let i = 0; i < t; i++) {
							for (let t = 0; t < A; t++) {
								if (((P += e[g] << (7 - D)), 7 == D || t == A - 1)) {
									let A = P.toString(16).toUpperCase()
									P < 16 && (A = '0' + A), (w += A), (P = 0), (D = -1)
								}
								g++, D++
							}
							w += '\n'
			...
			463: (A, t, e) => {
				...
				A.exports = {
					SizeType: e(345),
					Rotation: e(643),
					PrintDensityName: e(767),
					FontFamilyName: e(969),
					BarcodeTypeName: e(28),
					AlignmentValue: e(408),
					FontFamilyDefinition: g,
					LabelTools: e(22),
					ImageProcessor: e(202),
					ImageResizer: e(940),
					BarcodeRenderer: e(192),
					Size: e(667),
					Spacing: e(537),
					GridPosition: e(492),
					FontFamily: e(612),
			...
			265: (A) => {
				A.exports = class {
					constructor(A) {
						;(this.typeName = 'Alignment'), (this.value = A)
					}
					toString() {
						return this.value
					}
			...
			217: (A) => {
				A.exports = class {
					constructor(A) {
						;(this.typeName = 'BarcodeType'), (this.value = A)
					}
					toString() {
						return this.value
					}
			...
			612: (A, t, e) => {
				...
				A.exports = class {
					constructor(A) {
						;(this.typeName = 'FontFamily'), (this.value = A)
					}
					get definition() {
						return g[this.value]
					}
			...
			818: (A) => {
				A.exports = class {
					constructor(A) {
						;(this.typeName = 'PrintDensity'), (this.value = A)
					}
					toString() {
			...
			667: (A, t, e) => {
				...
				A.exports = class {
					constructor(A, t) {
						;(this.typeName = 'Size'),
							(this.value = A || 0),
							(this.sizeType = t || g.Absolute)
					}
					getValue(A) {
						return 'number' == typeof A && this.sizeType == g.Relative
							? this.value * A
							: this.value
					}
					toString() {
						return this.value + (this.sizeType == g.Relative ? '*' : '')
					}
				}
			},
			537: (A) => {
				A.exports = class {
					constructor(A, t, e, g) {
						;(this.typeName = 'Spacing'),
							(this.left = A || 0),
							(this.top = null == t ? this.left : t),
							(this.right = null == e ? this.left : e),
							(this.bottom = null == g ? this.top : g)
					}
					get horizontal() {
		...
		(function e(g) {
			...
			var w = (t[g] = { exports: {} })
			return A[g](w, w.exports, e), w.exports
		})(463)
	)
	var A, t
})
